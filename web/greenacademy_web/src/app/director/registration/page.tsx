...
export default function DirectorRegistrationPage() {
  ...
  const academies = useMemo(
    () => (Array.isArray(session?.academyNumbers) ? session!.academyNumbers.filter((n) => Number.isFinite(n)) : []),
    [session]
  );
  ...
  return (
    <main className="max-w-5xl mx-auto p-6 space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">등록 관리</h1>
          <p className="text-sm text-gray-600">학원 연결 승인 요청을 처리하세요.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            {academies.length === 0 && <option value="">학원번호 없음</option>}
            {academies.map((n) => (
      ...
      {err && <div className="rounded-lg bg-red-50 text-red-700 text-sm px-3 py-2 ring-1 ring-red-200">오류: {err}</div>}

      <section className="rounded-2xl border bg-white shadow-sm ring-1 ring-black/5">
        <div className="px-5 py-4 flex items-center justify-between">
          <div className="text-sm font-semibold text-gray-900">대기 중 요청 {list.length}건</div>
          <div className="flex items-center gap-2">
            <label className="text-xs text-gray-600">처리 메모(선택)</label>
            <input
              value={memo}
        ...
        {(!hasData && !loading && !err) ? (
          <div className="px-5 pb-5 text-sm text-gray-700">대기 중인 요청이 없습니다.</div>
        ) : (
