...
function NavTabs({
  ...
  const right =
    role === "director"
      ? (["관리", "출결확인", "Q&A", "공지사항", "가이드"] as const) // 원장: ‘출결확인’
...
/** ✅ HEAD 버전 SidebarProfile (환경설정/최근QnA 버튼 포함) */
...
export default function GreenAcademyDashboard() {
  ...
  const handleTab = (t: string) => {
    ...
    if (t !== "관리") setManageMenu(null); // 관리 벗어나면 드롭다운 선택 해제
  };
