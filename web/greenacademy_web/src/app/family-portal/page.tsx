...
function NavTabs({
  ...
  const tabsLeft = ["종합정보"] as const;
  const tabsRight = ["시간표", "Q&A", "공지사항", "가이드"] as const;
...
function TeacherManagePanel({ user }: { user: NonNullable<LoginResponse> }) {
  ...
  const togglePick = (sid: string) => setSelected((prev) => (prev.includes(sid) ? prev.filter((x) => x !== sid) : [...prev, sid]));
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-white ring-1 ring-black/5 shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-3">반 관리</h2>
        ...
        <div className="bg-gray-50/60 border rounded p-4 space-y-4">
          ...
          <div className="flex flex-wrap gap-3 items-end">
            <div>
              ...
              <input value={className} onChange={(e) => setClassName(e.target.value)} className="border rounded px-2 py-1" />
            </div>
            <div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              />
              <button onClick={search} className="px-3 py-1.5 rounded bg-gray-800 text-white">
                검색
              </button>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {hits.map((h) => {
                ...
                return (
                  <button
                    key={h.studentId}
                    onClick={() => togglePick(h.studentId)}
            ...
            {selected.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {selected.map((sid) => (
                  ...
                  >
                    {sid}
                    <button onClick={() => togglePick(sid)} className="text-emerald-700 hover:underline">
                      ×
                    </button>
                  </span>
                ))}
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 mt-4">
          ...
          {!loading && items.length === 0 && <div className="text-sm text-gray-500">아직 생성된 반이 없습니다.</div>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <PanelGrid>
        ...
        >
          {loading ? <div className="text-sm text-gray-600">로딩 중…</div> : <WeekCalendar startHour={8} endHour={22} events={events} />}
        </Panel>
      </PanelGrid>
    </div>
  );
...
/** 🆕 QR 생성 패널 (관리 ▸ QR 생성) */
function QRGeneratorPanel({ user }: { user: NonNullable<LoginResponse> }) {
  const academyOptions = (user.academyNumbers || []) as number[];
  const [academyNumber, setAcademyNumber] = useState<number | undefined>(academyOptions[0]);

  // 학원 입구
  const [entranceNonce, setEntranceNonce] = useState<string>("");
  const [entranceUrl, setEntranceUrl] = useState<string>("");

  // 강의실 입구
  const [roomNumber, setRoomNumber] = useState<number | undefined>(undefined);
  const [roomNonce, setRoomNonce] = useState<string>("");
  ...
  return (
    <div className="rounded-2xl bg-white ring-1 ring-black/5 shadow-sm p-6 space-y-6">
      ...
      <details className="rounded-xl border ring-1 ring-black/5 bg-white overflow-hidden" open>
        ...
        <div className="p-4 space-y-3">
          <div className="flex flex-wrap items-end gap-3">
            <div>
              <label className="block text-sm text-gray-600">Nonce(선택)</label>
              <input
                value={entranceNonce}
                onChange={(e) => setEntranceNonce(e.target.value)}
                placeholder="재사용 방지 토큰"
          ...
          {entranceUrl && (
            <div className="flex items-start gap-4">
              <img src={entranceUrl} alt="Academy Entrance QR" className="w-40 h-40 rounded-lg ring-1 ring-black/5" />
              <div className="space-y-2">
                <div className="text-sm text-gray-700">스캔 시: <code className="text-xs">type=academy</code> 이벤트로 처리</div>
        ...
        <div className="p-4 space-y-3">
          ...
          {roomUrl && (
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-2">
                <div className="text-sm text-gray-700">스캔 시: <code className="text-xs">type=room → waiting_room 등록</code></div>
        ...
        <div className="p-4 space-y-3">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div>
              ...
              <input
                value={seatNonce}
                onChange={(e) => setSeatNonce(e.target.value)}
                className="border rounded px-2 py-1 w-56"
              />
            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-2">
                <div className="text-sm text-gray-700">
                  스캔 시: <code className="text-xs">type=seat → waiting_room ⟶ seat 배치 & waiting_room 제거</code>
                </div>
                <Download url={seatUrl} name={`qr_seat_${academyNumber}_${seatRoom}_${seatNumber}.png`} />
              </div>
            </div>
          )}
        </div>
      ...
      <div className="text-xs text-gray-500">
        * 현재는 프로토타입으로 <code>api.qrserver.com</code>을 사용해 미리보기 이미지를 생성합니다.
        추후 서버 서명/유효기간/암호화(예: JWS) 등을 포함해 교체하세요.
      </div>
    </div>
  );
}
...
export default function GreenAcademyDashboard() {
  ...
  const [manageMenu, setManageMenu] = useState<"내정보" | "학생관리" | "반 관리" | "학원관리" | "QR 생성" | null>(null);
  ...
  useEffect(() => {
    if (!user) return;
    if ((user.role === "student" || user.role === "parent") && Array.isArray(user.academyNumbers) && user.academyNumbers.length > 0) {
      ...
      <main className="max-w-7xl mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
        ...
        {activeTab === "관리" && (
          <>
            ...
            {/* 🆕 QR 생성 */}
            {manageMenu === "QR 생성" && user && <QRGeneratorPanel user={user} />}
          </>
        )}
    ...
    >
      ...
      <div className="border border-black rounded p-4 space-y-4 bg-white">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button onClick={create} className="ml-auto px-4 py-2 rounded bg-black text-white">
            반 만들기
          </button>
        </div>
          ...
          {rooms.map((room) => {
            ...
            return (
              <div key={`${room.academyNumber}-${room.roomNumber}`} className="relative border border-black rounded-xl p-4 bg-white">
                {/* 우상단 X */}
                <button onClick={() => remove(room)} className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-black text-white text-sm">
                  ×
                </button>
                ...
                <RoomGridEditor rows={rowsCalc} cols={colsCalc} value={seats} onChange={(v) => saveLayout(room, v)} />
                <div className="text-xs text-black mt-2">변경 즉시 저장</div>
              </div>
            );
          })}
          {!loading && rooms.length === 0 && <div className="text-sm text-black">아직 생성된 강의실이 없습니다.</div>}
        </div>
      </div>
    </Panel>
  );
