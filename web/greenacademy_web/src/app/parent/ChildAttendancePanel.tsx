// src/app/parent/ChildAttendancePanel.tsx
"use client";
...
function clampDateToMonth(d: Date, month: Date) {
  // 선택한 날짜가 월을 이동해도 유지되도록, 해당 월에 같은 일자가 없으면 말일로 맞춤
...
function MiniDatePicker({
  ...
  const [val, setVal] = useState<string>(formatDateInputValue(value));

  useEffect(() => {
    setVal(formatDateInputValue(value));
  }, [value]);
...
export default function ChildAttendancePanel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!selectedChild) {
          const first = list?.[0]?.studentId ?? null;
  ...
  // 선택된 자녀의 출결 로드
  useEffect(() => {
    if (!selectedChild) { setRows([]); setErr(null); return; }
    let aborted = false;
    (async () => {
      setLoading(true);
      setErr(null);
      try {
        const data = await apiGet<AttendanceRow[]>(
          `/api/students/${encodeURIComponent(selectedChild)}/attendance`,
          token
        );
        if (aborted) return;
        setRows(Array.isArray(data) ? data : []);
      } catch (e: any) {
        if (!aborted) setErr(e?.message ?? "출결 정보를 불러오지 못했습니다.");
        if (!aborted) setRows([]);
      } finally {
  ...
  const byDate = useMemo(() => {
    ...
    for (const r of rows) {
      const ymd = parseToYmd(r.date);
      map.set(ymd, [...(map.get(ymd) ?? []), r]);
    }
    return map;
  }, [rows]);

  // 현재 월에 표시할 날짜 그리드(일요일 시작 6줄)
  const daysForGrid = useMemo(() => {
    const start = startOfMonth(month);
    const end = endOfMonth(month);
    const firstWeekday = start.getDay(); // 0: Sun
    const total = firstWeekday + end.getDate();
    const rowCount = Math.ceil(total / 7);
    const out: Date[] = [];
    for (let i = 0; i < firstWeekday; i++) {
      out.push(new Date(start.getFullYear(), start.getMonth(), start.getDate() - (firstWeekday - i)));
    }
    for (let d = 1; d <= end.getDate(); d++) {
      out.push(new Date(start.getFullYear(), start.getMonth(), d));
    }
    while (out.length < rowCount * 7) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg md:text-xl font-semibold text-gray-900">자녀 출결 확인</h2>
          <p className="text-sm text-gray-700 mt-1">자녀를 선택하고, 달력에서 날짜를 눌러 해당일의 출결을 확인하세요.</p>
        </div>
      </div>
        ...
        <div className="rounded-2xl bg-white ring-1 ring-black/5 shadow-sm p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-gray-700">
            <span className="inline-flex items-center gap-1">
              <span className="inline-block w-3 h-3 rounded-full bg-emerald-400" /> 출석
            </span>
            <span className="inline-flex items-center gap-1">
              <span className="inline-block w-3 h-3 rounded-full bg-amber-400" /> 지각
            </span>
            <span className="inline-flex items-center gap-1">
              <span className="inline-block w-3 h-3 rounded-full bg-red-400" /> 결석
            </span>
          </div>
        </div>
