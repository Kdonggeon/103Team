...
// ======================================================
async function requestAuthLocal<T = any>(
  path: string,
  init: RequestInit = {}
): Promise<T> {
...
export async function getOrCreateStudentRoom(
  academyNumber: number
): Promise<QnaId> {
  return requestAuthLocal(
    `/backend/api/questions/room?academyNumber=${academyNumber}`,
    { method: "GET" }
  );
}

// 학부모 방 (우선 통합 경로 시도 → 실패 시 전용 경로)
export async function getOrCreateParentRoom(
  academyNumber: number
): Promise<QnaId> {
  try {
    return await requestAuthLocal(
      `/backend/api/questions/room?academyNumber=${academyNumber}`,
      { method: "GET" }
    );
  } catch {
    return requestAuthLocal(
      `/backend/api/questions/room/parent?academyNumber=${academyNumber}`,
      { method: "GET" }
    );
  }
}
...
export async function getOrCreateRoomAuto(
  academyNumber: number
): Promise<QnaId> {
...
export async function getQuestion(questionId: string): Promise<QnaQuestion> {
  const q = await requestAuthLocal<QnaQuestion>(
    `/backend/api/questions/${questionId}`,
    { method: "GET" }
  );
...
export async function getAnswers(
  ...
  return requestAuthLocal(
    `/backend/api/questions/${questionId}/answers`,
    { method: "GET" }
  );
}

// 읽음 처리 (★ 서버 갱신 + 로컬 lastReadAt 동기화)
export async function markQuestionRead(
  questionId: string
): Promise<void> {
  await requestAuthLocal(
    `/backend/api/questions/${questionId}/read`,
    { method: "PUT" }
  );
 ...
 * 1) /backend/api/questions/{id}/followups
 * 2) /backend/api/questions?parentId={id}
 */
export async function getFollowupsFlexible(
  rootId: string
): Promise<any[]> {
  try {
    const a = await requestAuthLocal<any[]>(
      `/backend/api/questions/${rootId}/followups`,
      { method: "GET" }
    );
  ...
  try {
    const b = await requestAuthLocal<any[]>(
      `/backend/api/questions?parentId=${encodeURIComponent(rootId)}`,
      { method: "GET" }
    );
...
export async function postFollowupFlexible(
  ...
  try {
    return await requestAuthLocal(
      `/backend/api/questions/${parentQuestionId}/followups`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content }),
  ...
  return requestAuthLocal(`/backend/api/questions`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function getRecentQna(): Promise<QnaRecent | null> {
  try {
    const data = await requestAuthLocal<QnaRecent>(
      `/backend/api/qna/recent`,
      { method: "GET" }
    );
...
export async function openRecentQna(router: { push: (href: string) => void }) {
  ...
  if (recent?.questionId) {
    router.push(
      `/family-portal?tab=qna&qnaId=${encodeURIComponent(recent.questionId)}`
    );
  } else {
    alert("최근 QnA가 없습니다.");
  }
...
export async function getStudentsForAcademy(
  ...
  const tryPaths = [
    `/backend/api/teachers/students?academyNumber=${academyNumber}`, // 우선 권장
    `/backend/api/students?academyNumber=${academyNumber}`, // 대안 1
    `/backend/api/classes?academyNumber=${academyNumber}`, // 대안 2 (수업에서 학생 집계)
  ];
      ...
      if (Array.isArray(res?.content) || Array.isArray(res?.items)) {
        ...
        for (const c of arr) {
          ...
          for (const s of sts) {
            ...
            if (!bag[id]) {
              bag[id] = { id, name: s?.name ?? s?.studentName };
            }
          }
        }
...
export async function getOrCreateTeacherRoom(
  ...
  if (!academyNumber || !studentId)
    throw new Error("학원 또는 학생 선택이 필요합니다.");
  const res = await requestAuthLocal(
    `/backend/api/questions/room?academyNumber=${academyNumber}&studentId=${encodeURIComponent(
      studentId
    )}`,
    { method: "GET" }
  );
  if (!res || !(res.id || res._id))
    throw new Error("질문 방을 찾거나 생성하지 못했습니다.");
  return res;
}
...
export async function listQuestions(
  ...
  const raw = await requestAuthLocal<QnaQuestion[]>(
    `/backend/api/questions${qs}`,
    { method: "GET" }
  );
  return Array.isArray(raw)
    ? raw.map((x) => ({ ...x, unreadCount: x?.unreadCount ?? 0 }))
    : [];
}
...
export async function getOrCreateTeacherRoomById(
  ...
  return requestAuthLocal(
    `/backend/api/questions/room/by-id?academyNumber=${academyNumber}&id=${encodeURIComponent(
      targetId
    )}`,
    { method: "GET" }
  );
...
function scoreForRecent(
  i: QnaListItem & Partial<QnaQuestion>
): number {
  const base = Math.max(
...
export function pickLatestTarget(
  ...
  return [...pool].sort(
    (a, b) => scoreForRecent(b) - scoreForRecent(a)
  )[0];
}

// (선택) 목록 정렬 헬퍼
export function sortByCounterpartyLatest<
  T extends Partial<QnaQuestion>
>(arr: T[]): T[] {
  return [...arr].sort(
    (a, b) => scoreForRecent(b as any) - scoreForRecent(a as any)
  );
}
